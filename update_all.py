...
def run_command(cmd, description):
    print(f"\n🔄 [{description}] を実行中...")
    try:
        # capture_outputを外し、画面に直接表示＆入力できるようにする
...
# STEP 1: 前日の確定結果＆的中履歴の更新 (※ここで日付入力プロンプトが出ます)
if os.path.exists("update_results_chiho.py"):
    run_command("python update_results_chiho.py", "確定着順＆結果データの更新")

# 🌟 追加 STEP 2: 昨日の予想の答え合わせ・成績集計
if os.path.exists("evaluate_yesterday.py"):
    run_command("python evaluate_yesterday.py", "昨日の成績集計・答え合わせ")

# 🌟 追加 STEP 3: 新AI（第3形態）の再学習・脳みそアップデート
if os.path.exists("train_ensemble_model.py"):
    run_command("python train_ensemble_model.py", "第3形態AIのアンサンブル学習(脳みそ更新)")

# STEP 4: 本日の最新出馬表データのスクレイピング取得
